...
(function () {
  ...
  /* ══════════════════════════════════════════════════════════════
     i18n ENGINE — English / 中文 toggle
     ══════════════════════════════════════════════════════════════ */

  // All Chinese translations: key → Chinese string
  // Keys match data-i18n-key attributes in module HTML
  window.TRANSLATIONS = {};

  // Set language: 'en' or 'zh', persists to localStorage
  window.setLang = function (lang) {
    document.documentElement.dataset.lang = lang;
    localStorage.setItem('course-lang', lang);
    // Swap text on all i18n elements
    $$('[data-i18n-key]').forEach(function (el) {
      var key = el.dataset.i18nKey;
      var zh  = el.dataset.i18nZh;
      if (lang === 'zh' && zh) {
        el.textContent = zh;
      } else {
        el.textContent = el.dataset.i18nEn || el.textContent;
      }
    });
  ...
  // Inject toggle button into nav on load
  document.addEventListener('DOMContentLoaded', function () {
    var nav = $('#nav .nav-inner');
    if (!nav) return;
    ...
    var title = $('.nav-title', nav);
    if (title) title.parentNode.insertBefore(btn, title.nextSibling);
    // Restore saved language
    var saved = localStorage.getItem('course-lang');
    if (saved === 'zh') window.setLang('zh');
  });
